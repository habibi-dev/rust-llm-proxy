...
- **State management** – `core::state` keeps application state behind a thread-safe `OnceCell`, while `utility::state::app_state()` exposes a typed accessor to avoid tight coupling.
